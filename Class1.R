...
### Box Plot - Age
age_boxplot <- ggplot(data1, aes(y = Age)) +
...
MonthlyIncome_boxplot <- ggplot(data1, aes(y = MonthlyIncome)) +
  geom_boxplot(fill = "skyblue", color = "black") 
...
numerical_features <- data[, c("Age","YearsAtCompany","MonthlyRate","MonthlyIncome","TotalWorkingYears")]
